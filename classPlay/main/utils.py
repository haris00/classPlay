...
def user_redirect(current_user):
    ...
    if user_type is "Student":
        return redirect(url_for('professor.professor_account', username=current_user.userName))
    elif user_type is "Professor":
        return redirect(url_for('student.student_account', username=current_user.userName))
